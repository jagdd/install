...
GH_PROXY=''  # 留空直连GitHub，或设置为 'https://ghproxy.com/'
...
log_action() {
  local msg="$1"
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] $msg" >> "${LOG_DIR}/installer.log"
}
...
backup_config() {
  if [ -f "$WORK_DIR/config.json" ]; then
    ...
    # 只保留最近10个备份
    ls -t "${BACKUP_DIR}"/config_*.json | tail -n +11 | xargs -r rm -f
  fi
}
...
EOF
  ...
  ' "$CONF_DIR"/*.json > "$WORK_DIR/config.json" || {
    warn "⚠️ jq 合并失败，使用上次的配置"
  }

  # 校验 JSON
  jq . "$WORK_DIR/config.json" >/dev/null 2>&1 || {
    err "❌ 合并后的配置无效，保留上次有效配置"
  }
}
...
EOF
  ...
  merge_config
  svc_restart
  log_action "Shadowsocks 安装完成，密码: ${SS_PASS}"
...
enable_bbr() {
  ...
  if [ "$current_cc" = "bbr" ]; then
    ...
    warn "⚠️ BBR 启用可能失败，当前拥塞控制算法: $current_cc"
  fi
...
uninstall_all() {
  echo ""
  warn "⚠️  即将卸载 sing-box 及其所有配置与服务文件。"
...
restore_backup() {
  ...
  for backup in "${backups[@]}"; do
    local timestamp=$(basename "$backup" | sed 's/config_\(.*\)\.json/\1/')
    echo "$i) $timestamp"
    ((i++))
  done
...
# 执行主程序
main https://api.ip.sb/ip || echo "YOUR_IP")
